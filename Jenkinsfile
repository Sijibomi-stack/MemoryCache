...
def configuration = [vaultUrl: 'http://10.32.0.8:8200',  vaultCredentialId: 'vault-approle', engineVersion: 1]
...
pipeline {
  ...
  environment{
    PRIVATE_TOKEN = vault path: 'secrets/jenkins/github', key: 'private-token', vaultUrl: 'http://10.32.0.8:8200', credentialsId: 'vault-approle', engineVersion: "1"
    USERNAME = vault path: 'secrets/jenkins/github', key: 'username', vaultUrl: 'http://10.32.0.8:8200', credentialsId: 'vault-approle', engineVersion: "1"
    REPO_NAME = vault path: 'secrets/jenkins/github', key: 'github-username', vaultUrl: 'http://10.32.0.8:8200', credentialsId: 'vault-approle', engineVersion: "1"
  }
  stages {
     ...
     stage('Build Memory Cache Project') {
       steps {
         container('kaniko') {
		    wrap([$class: 'VaultBuildWrapper', configuration: [engineVersion: 1, vaultCredentialId: 'vault-approle', vaultUrl: 'http://10.32.0.8:8200'], vaultSecrets: [[path: 'secrets/jenkins/github', secretValues: [[envVar: 'PRIVATE_TOKEN', vaultKey: 'private-token'], [envVar: 'USERNAME', vaultKey: 'username'], [envVar: 'REPO_NAME', vaultKey: 'github-username']]]]]) {
